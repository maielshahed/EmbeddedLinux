...
class LaneDetector:
    ...
    def check_car_position(self, frame):  
        ...
        occupation_ratio = non_zero_pixels / total_pixels if total_pixels != 0 else 0  

        # Return True if there's significant object detection  
        return occupation_ratio > 0.2  
    ...
    def draw_parking_guidelines(self, frame):  
        ...
        # Define guideline points (proportional to frame size)  
        left_top       = (int(width * 0.35), int(height * 0.6))  
        right_top      = (int(width * 0.65), int(height * 0.6))  
        left_bottom    = (int(width * 0.2),  height)  
        right_bottom   = (int(width * 0.8),  height)  

        # Center vertical line  
        center_top     = (int(width * 0.5),  int(height * 0.6))  
        center_bottom  = (int(width * 0.5),  height)  

        # Check car position to determine line color  
        is_car_inside = self.check_car_position(frame_copy)  
        line_color = (5, 168, 19) if is_car_inside else (0, 32, 232)  # Green or Red  

        # Draw guidelines  
        cv2.line(frame_copy, left_bottom,   left_top,     line_color, 2)  
        cv2.line(frame_copy, right_bottom,  right_top,    line_color, 2)  
        cv2.line(frame_copy, center_bottom, center_top,  (255, 0, 0), 2)  

        # Draw horizontal reference lines  
        cv2.line(frame_copy, (int(width * 0.2), int(height * 0.6)),  
                            (int(width * 0.8), int(height * 0.6)),  
                            line_color, 2)  
        cv2.line(frame_copy, (int(width * 0.2), int(height * 0.8)),  
